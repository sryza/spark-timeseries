...
class RebaseSuite extends FunSuite with ShouldMatchers {
  test("iterateWithUniformFrequency single value") {
    val baseDT = ZonedDateTime.of(2015, 4, 8, 0, 0, 0, 0, ZoneId.UTC)
  ...
  test("uniform source same range") {
    ...
    val source = uniform(ZonedDateTime.of(2015, 4, 8, 0, 0, 0, 0, ZoneId.UTC),
      vec.length,
      1.businessDays)
    val target = source
  ...
  test("uniform source, target fits in source") {
    ...
    val source = uniform(ZonedDateTime.of(2015, 4, 8, 0, 0, 0, 0, ZoneId.UTC),
      vec.length,
      1.businessDays)
    val target = uniform(ZonedDateTime.of(2015, 4, 9, 0, 0, 0, 0, ZoneId.UTC),
      5,
      1.businessDays)
    val rebased = rebase(source, target, vec, NaN)
  ...
  test("uniform source, target overlaps source ") {
    ...
    val source = uniform(ZonedDateTime.of(2015, 4, 8, 0, 0, 0, 0, ZoneId.UTC),
      vec.length,
      new DayFrequency(1))
    val targetBefore = uniform(ZonedDateTime.of(2015, 4, 4, 0, 0, 0, 0, ZoneId.UTC),
      8,
      new DayFrequency(1))
    val targetAfter = uniform(ZonedDateTime.of(2015, 4, 11, 0, 0, 0, 0, ZoneId.UTC),
      8,
      new DayFrequency(1))
  ...
  test("uniform source, source fits in target") {
    ...
    val source = uniform(ZonedDateTime.of(2015, 4, 8, 0, 0, 0, 0, ZoneId.UTC),
      vec.length,
      1.businessDays)
    val target = uniform(ZonedDateTime.of(2015, 4, 7, 0, 0, 0, 0, ZoneId.UTC),
      8,
      1.businessDays)
    val rebased = rebase(source, target, vec, NaN)
    assertArraysEqualWithNaN(
  ...
  test("irregular source same range") {
    ...
    val source = irregular((4 until 10).map(d =>
      ZonedDateTime.of(2015, 4, d, 0, 0, 0, 0, ZoneId.UTC)).toArray)
    vec.size should be (source.size)
    val target = uniform(ZonedDateTime.of(2015, 4, 4, 0, 0, 0, 0, ZoneId.UTC),
      vec.length,
      new DayFrequency(1))
    val rebased = rebase(source, target, vec, NaN)
    rebased should be (vec)
  ...
  test("irregular source, hole gets filled default value") {
    val dt = ZonedDateTime.of(2015, 4, 10, 0, 0, 0, 0, ZoneId.UTC)
  ...
  test("irregular source, target overlaps source ") {
    val dt = ZonedDateTime.of(2015, 4, 10, 0, 0, 0, 0, ZoneId.UTC)
    val source = irregular(Array(dt, dt.plusDays(1), dt.plusDays(3)))
    val targetBefore = uniform(ZonedDateTime.of(2015, 4, 8, 0, 0, 0, 0, ZoneId.UTC),
      4,
      new DayFrequency(1))
    val vec = new DenseVector(Array(1.0, 2.0, 3.0))
  ...
  test("irregular source, irregular target") {
    ...
    cases.foreach { case (source, target, expected) =>
      val sourceIndex = irregular(source.map(x => ZonedDateTime.of(2015, 4, x, 0, 0, 0, 0, ZoneId.system)))
      val targetIndex = irregular(target.map(x => ZonedDateTime.of(2015, 4, x, 0, 0, 0, 0, ZoneId.system)))
