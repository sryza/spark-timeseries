...
import breeze.linalg.{Transpose, DenseMatrix}
...
class RegressionARIMASuite extends  FunSuite with ShouldMatchers{
  ...
  test("Cochrane-Orcutt-Stock-Data") {
    ...
    val expenditure : Array[Double] = Array(214.6,217.7,219.6,227.2,230.9,233.3,234.1,232.3,233.7,236.5,238.7,243.2,249.4,254.3,260.9,263.3,265.6,268.2,270.4,275.6)

    val stock: Array[Double] = Array(159.3,161.2,162.8,164.6,165.9,167.9,168.3,169.7,170.5,171.6,173.9,176.1,178.0,179.1,180.2,181.2,181.6,182.5,183.3,184.3)
    val Y: linalg.DenseVector[Double] = linalg.DenseVector(expenditure)
