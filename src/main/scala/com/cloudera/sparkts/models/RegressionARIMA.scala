...
import org.apache.commons.math3.stat.regression.{SimpleRegression, OLSMultipleLinearRegression}
...
object RegressionARIMA {
  ...
  def fitCochraneOrchutt(ts:linalg.Vector[Double],regressors: DenseMatrix[Double],maxIter: Int =10) : RegressionARIMAModel =  {
    ...
    (0 until regressors.rows).map(row=> {regressorArr(row) = regressors(row,::).inner.toArray })
    ...
    while(!finished) {
      ...
      val rhosConverged = if(iter >=2) {Math.abs(rhos(iter-1)-rhos(iter-2))<=rhoDiffThr} else false
