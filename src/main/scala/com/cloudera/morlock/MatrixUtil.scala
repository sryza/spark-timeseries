...
private[morlock] object MatrixUtil {
   ...
   */
  private[morlock] def fromBreeze(breeze: Matrix[Double]): SM = {
    breeze match {
   ...
   * @return a spark-mllib Matrix instance
   */
  private[morlock] def toBreeze(sparkMatrix: SM): Matrix[Double] = {
    sparkMatrix match {
      case dm: SDM =>
        if (!dm.isTransposed) {
   ...
   */
  private[morlock] def fromBreeze(breezeVector: Vector[Double]): SV = {
    breezeVector match {
   ...
   */
  private[morlock] def toBreeze(sparkVector: SV): Vector[Double] = {
    sparkVector match {
      case v: SDV =>
  ...
  private[morlock] implicit def mSparkToBreeze(sparkMatrix: SM): Matrix[Double] =
    toBreeze(sparkMatrix)

  private[morlock] implicit def dmSparkToBreeze(sparkMatrix: SDM): DenseMatrix[Double] =
    toBreeze(sparkMatrix).asInstanceOf[DenseMatrix[Double]]

  private[morlock] implicit def mBreezeToSpark(breezeMatrix: Matrix[Double]): SM =
    fromBreeze(breezeMatrix)

  private[morlock] implicit def dmBreezeToSpark(breezeMatrix: DenseMatrix[Double]): SDM =
    fromBreeze(breezeMatrix).asInstanceOf[SDM]

  private[morlock] implicit def vSparkToBreeze(sparkVector: SV): Vector[Double] =
    toBreeze(sparkVector)

  private[morlock] implicit def dvSparkToBreeze(sparkVector: SDV): DenseVector[Double] =
    toBreeze(sparkVector).asInstanceOf[DenseVector[Double]]

  private[morlock] implicit def vBreezeToSpark(breezeVector: Vector[Double]): SV =
    fromBreeze(breezeVector)

  private[morlock] implicit def dvBreezeToSpark(breezeVector: DenseVector[Double]): SDV =
    fromBreeze(breezeVector).asInstanceOf[SDV]

  private[morlock] implicit def fvtovBreezeToSpark(f: (Vector[Double]) => Vector[Double])
    : (SV) => SV = {
    v: SV => f(v)
  }
