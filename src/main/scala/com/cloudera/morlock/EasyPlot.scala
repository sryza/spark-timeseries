...
import com.cloudera.morlock.models.Autoregression
...
object EasyPlot {
  ...
  private[morlock] def calcConfVal(conf: Double, n: Int): Double = {
    val stdNormDist = new NormalDistribution(0, 1)
    val pVal = (1 - conf) / 2.0
    stdNormDist.inverseCumulativeProbability(1 - pVal) / Math.sqrt(n)
  }

  private[morlock] def drawCorrPlot(corrs: Array[Double], confVal: Double, p: Plot): Unit = {
